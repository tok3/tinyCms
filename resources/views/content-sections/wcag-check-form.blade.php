<section class="position-relative p-4  @if(!empty($data['border-top'])) border-top @endif mt-4 @if(!empty($data['border-bottom'])) border-bottom @endif">
    ...
    <div class="container ">
                    ...
                    <form novalidate="" class="needs-validation">
                        <div class="d-flex flex-column flex-md-row">
                            <div class="mb-2 flex-grow-1 mb-md-0 me-md-2">
                                <label for="urlInput" id="urllabel" class="shrink mobile-off">Zu prüfende URL eingeben</label>
        ...
        <div id="summaryOutput" class="mt-5" style="display:none;" data-aos="fade-right">
            ...
            @if(!empty($data['showCta']) && ($data['ctaPosition'] ?? 'after-form') === 'after-result')
                @include('content-sections._wcag-check-cta', ['data' => $data])
            @endif
            ...
            <!-- cta -->
            @if(!empty($data['showCta']) && ($data['ctaPosition'] ?? 'after-form') === 'after-form')
                @include('content-sections._wcag-check-cta', ['data' => $data])
            @endif
            <!-- ende cta -->
        </div>
