...
<p>
        {{ $declaration->declaration_intro_text_ez }}
    </p>

    <h3>Beschreibung der Dienstleistung</h3>

    <p>
        {{ $company->company_offer_ez }}
    </p>
    ...
    @if(empty($issues))
    <p>
        {{ $declaration->bfsg_full_ez }}
    </p>
    @else
    <p>
        {{ $declaration->bfsg_partial_ez }}
    </p>
    <p><strong>{{ $declaration->non_conform_content_ez }}</strong></p>
    <ul>
    ...
    @if($declaration->market_surveillance_board_address !== null && $declaration->market_surveillance_board_address !== ''    )
        ...
        <p>
            {{ $declaration->market_surveillance_board_address_text_ez }}
