<x-filament::page>
    ...
    <div class="space-y-4">
        ...
        @include('filament.resources.pa11y-accessibility-issues.partials.head', [
            'slugGrouped' => $slugGrouped,
            'slugIndex' => $slugIndex
        ])
        ...
        <div class="space-y-8">
            ...
            @php
                $data = $this->getGroupedRecords();
            @endphp

            @foreach ($data['groups'] as $code => $issues)
                ...
                <div class="issue-group bg-white dark:bg-gray-800 border-l-4 shadow p-4 rounded mb-4
                        ...
                        <span class="text-sm font-bold uppercase flex items-center
                            ...
                            @else
                                <svg class="w-5 h-5 mr-2" fill="currentColor" viewBox="0 0 24 24">
                                    ...
                                    <text x="12" y="16" text-anchor="middle" font-size="12" font-weight="bold">!</text>
                                </svg>
                            @endif
                    ...
                    <h2 class="mt-5 mb-3 text-xl font-medium text-gray-900 dark:text-white">
                        {{ $issues->first()->translated_message }}
                    </h2>
        ...
        <!-- Pagination -->
        <div>
            {{ $data['paginator']->appends(request()->query())->links() }}
        </div>
